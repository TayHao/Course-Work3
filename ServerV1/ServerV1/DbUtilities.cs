...
WHERE Login = @Login AND Password = @Password", connection))
        ...
        public static Dictionary<string, int> GetSubjets()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
        ...
        public static Dictionary<string, int> GetSubjects(int instructorId)
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            using (SqlCommand query = new SqlCommand("SELECT SubjectId, Name FROM [Subject] WHERE InstructorId = @Id", connection))
            {
            ...
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                using (SqlCommand count = new SqlCommand("SELECT COUNT(*) FROM Event WHERE ProjectId = @ProjectId AND SerialNumber = @SerialNumber", connection))
                {
                    ...
                    if ((int)count.ExecuteScalar() > 0)
                        throw new ArgumentException("SerialNumber вже існує");
                }
                using (SqlTransaction transaction = connection.BeginTransaction())
                        ...
                        while (reader.Read())
                        {
                            ...
                            switch (projectType)
                            {
                                case EProjectType.LabWork:
                                    projects.Add(new LabWorks((int)reader["InstructorId"], (string)reader["Theme"], (string)reader["Description"])
                                    {
